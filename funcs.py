...
def generate_settings(level):
    ...
    return settings[level] | db.config.settings.SETTINGS_GAME


def load_setting():
    with open("values/profile_values.json", "r", encoding="utf-8") as f:
        user_setting = json.load(f)
        return user_setting

def dump_setting(setting):
    with open("values/profile_values.json", "w", encoding="utf-8") as f:
