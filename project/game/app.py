...
class App:
    def __init__(self, screen, session):
        ...
        print(self.base_dir)
        self.move_sounds = [
            pygame.mixer.Sound(str(self.base_dir / 'data' / 'music' / f"move_music_{i}.wav"))
            for i in range(1, 4)
        ]

        self.click_sound = pygame.mixer.Sound(self.base_dir / 'data' / 'music' / "click_music.wav")
    ...
    def render_decoration(self, draw_back_button=False, draw_ulta_red=False):
        ...
        # <===== Твой баланс =====>
        pygame.draw.rect(self.screen, style.S_TABLE_SCORE, (40, 10, 150, 60), 0, 5)
        pygame.draw.rect(
            self.screen, style.S_TABLE_SCORE_BORDER, (40, 10, 150, 60), 5, 5
        )
        ...
        self.screen.blit(col_text, (button_x_text, button_y_text))
        ...
        self.screen.blit(g_text, (button_x_text, button_y_text))
        # <===== Твой баланс =====>

        # <===== Ульта удаление =====>
        if draw_ulta_red:
            ulta_delete = pygame.draw.rect(
                self.screen, style.S_TABLE_SCORE, (90, 75, 50, 50), 0, 5
            )
            pygame.draw.rect(
                self.screen, style.S_TABLE_SCORE_BORDER, (90, 75, 50, 50), 5, 5
            )
            ...
            self.screen.blit(image, (ulta_delete.x + 5, ulta_delete.y + 5))
        else:
            ulta_delete = pygame.draw.rect(
                self.screen, style.S_TABLE_SCORE, (90, 75, 50, 50), 0, 5
            )
        ...
        # <===== Твои очки =====>
        pygame.draw.rect(self.screen, style.S_TABLE_SCORE, (250, 10, 100, 50), 0, 5)
        pygame.draw.rect(
            self.screen, style.S_TABLE_SCORE_BORDER, (250, 10, 100, 50), 5, 5
        )
        ...
        # <===== Лучшие очки =====>
        pygame.draw.rect(self.screen, style.S_TABLE_SCORE, (360, 10, 100, 50), 0, 5)
        pygame.draw.rect(
            self.screen, style.S_TABLE_SCORE_BORDER, (360, 10, 100, 50), 5, 5
        )
        font = pygame.font.SysFont("spendthrift", 20)
        ...
        font = pygame.font.SysFont("spendthrift", 30)
        text = font.render(
            str(self.best_score) if self.best_score else "", True, style.S_TABLE_SCORE_TEXT_VALUE
        )
        ...
        pygame.draw.rect(
            self.screen, style.RECT, (50, 150, 400, 400), 0, 15
        )
    ...
    def draw_cell_rect(self, rect, color):
        ...
        pygame.draw.rect(self.screen, border_color, rect, 0, 5)
    ...
    def render_cell_text(self, cell_value, i, j):
        ...
        self.screen.blit(text_rendered, (text_x, text_y))
    ...
    def get_cell(self, x, y):
        ...
        if (
                x < 50
                or x > 50 + self.cell_size * self.value + self.margin * (self.value + 1)
                or y < 150
                or y > 150 + self.cell_size * self.value + self.margin * (self.value + 1)
        ):
            return None
        return (
            (y - 150) // (self.cell_size + self.margin),
            (x - 50) // (self.cell_size + self.margin),
        )
    ...
    def update_settings_and_refresh(self, level):
        ...
        self.points = 0  # Счет
        self.steps = 0  # Количество шагов
        self.start_balance = funcs.gem_assist.get_balance()  # Баланс до игры
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key in (
                        ...
                        if self.is_lose(self.board):
                            self.render_cells()
                            # перекидываю на экран поражения
                            self.lose_page()
                        elif self.is_win(self.board):
                            self.render_cells()
                            # перекидываю на экран выйгрыша
                            self.win_page()
    ...
    def lose_page(self):
        pygame.display.flip()
        width, height = self.screen_size
        screen_filename = datetime.datetime.now().strftime("screen_save_%d_%m_%Y_%H_%M_%S.png")
        ...
        board_filename = datetime.datetime.now().strftime("board_save_%d_%m_%Y_%H_%M_%S.png")
        board_path = self.base_dir / "data" / "saves"
        board_full_path = board_path / board_filename

        funcs.crop_image(screen_full_path, (50, 150), (450, 550), board_full_path)
        ...
        text_label = font.render("ВЫ ПРОИГРАЛИ!", True, style.S_TEXT)

        button_x_text_label = width // 2 - text_label.get_width() // 2
        button_y_text_label = 70 - text_label.get_height() // 2

        self.screen.blit(text_label, (button_x_text_label, button_y_text_label))
        ...
        self.screen.blit(text_back, (button_x_text, button_y_text))
        # Назад

        # Счет
        font = pygame.font.SysFont("spendthrift", 35)
        text_label = font.render("Счет:", True, style.S_TITLE)
        ...
        # Ходов
        font = pygame.font.SysFont("spendthrift", 35)
        text_label = font.render("Ходов:", True, style.S_TITLE)
        ...
        # Заработано
        font = pygame.font.SysFont("spendthrift", 35)
        text_label = font.render("Заработано:", True, style.S_TITLE)
        ...
        # Счет value
        font = pygame.font.SysFont("spendthrift", 35)
        text_label = font.render(str(self.points), True, style.S_TEXT)
        ...
        # Ходов value
        font = pygame.font.SysFont("spendthrift", 35)
        text_label = font.render(str(self.steps), True, style.S_TEXT)
        ...
        text_label = font.render(str(g_fonted), True, style.S_TEXT)
        ...
        button_x_g_text = (
                300 + text_label.get_width() + 5
        )
        button_y_g_text = button_y_text_label

        self.screen.blit(g_text, (button_x_g_text, button_y_g_text))
        # Заработано value
        ...
        while True:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    pos = event.pos
                    x, y = pos
                    if button_back_rect.collidepoint(x, y):
                        self.click_sound.play()
                        self.choice_page()
                    ...
                    if button_back_rect.collidepoint(x, y):
                        ...
                        self.screen.blit(
                            text_back, (button_x_text, button_y_text)
                        )
                    else:
                        pygame.draw.rect(
                            self.screen,
    ...
    def win_page(self):
        ...
        font = pygame.font.SysFont("spendthrift", 50)
        text_label = font.render("ВЫ ВЫЙГРАЛИ!", True, style.S_TEXT)
    ...
    def start_page(self):
        ...
        self.screen.blit(text_play, (button_x_text_1, button_y_text_1))
        # <==== Играть ====>
        ...
        self.screen.blit(text_records, (button_x_text_2, button_y_text_2))
        # <==== Рекорды ====>
        ...
        self.screen.blit(text_rules, (button_x_text_3, button_y_text_3))
        # <==== Правила ====>
        ...
        while True:
            ...
            self.screen.blit(text_title, (text_x, text_y))
    ...
    def choice_page(self):
        ...
        Args:
            self.screen (pygame.Surface): The game self.screen.
        ...
        self.screen.blit(text_4_x_4, (button_x_text_1, button_y_text_1))
        # <==== 4 x 4 ====>
        ...
        self.screen.blit(text_6_x_6, (button_x_text_2, button_y_text_2))
        # <==== 6 x 6 ====>
        ...
        self.screen.blit(text_8_x_8, (button_x_text_3, button_y_text_3))
        # <==== 8 x 8 ====>
        ...
        self.screen.blit(text_back, (button_back_x_text, button_back_y_text))
        # <==== Назад ====>
    ...
    def records_page(self):
        ...
        font_title = pygame.font.SysFont("spendthrift", 60)
        text_title = font_title.render(
            "Рекорды", True, style.S_MAIN_TITLE
        )
        ...
        if top_games:
            for index, game in enumerate(top_games, 1):
                ...
                self.screen.blit(resized_image, (60, 130 * index))
                ...
                button_y_text_label = 15 + 130 * index - text_label.get_height() // 2

                self.screen.blit(text_label, (button_x_text_label, button_y_text_label))

                text_label = font.render(str(game.points), True, style.S_TEXT)
                ...
                text_label = font.render(str(game.steps), True, style.S_TEXT)
                ...
                # Результат
                text_label = font.render("Результат:", True, style.S_TITLE)
                ...
                text_label = font.render("Победа" if game.is_win else "Поражение", True, style.S_TEXT)
                ...
                # Когда
                text_label = font.render("Когда:", True, style.S_TITLE)
                ...
                text_label = font.render(game.datetime.strftime("%d.%m.%Y %H:%M"), True, style.S_TEXT)
            ...
            # <==== Надпись ====>
            font_title = pygame.font.SysFont("spendthrift", 50)
            text_title = font_title.render(
                "Увы, пока не было игр", True, style.S_TITLE
            )
    ...
    def rules_page(self):
        ...
        self.screen.blit(text, (button_x_text_1, button_y_text_1))
        # <==== Назад ====>
